    ...
    >
      ...
      >
        ...
        <div
          class="pl-4"
        >
          Selected fields you want to share with the selected network
          ...
          >
            <b-form-select
              ...
              :options="getAvailableDownstreamServers"
        ...
        >
          <b-form-group>
            <b-form-select
              ...
              :options="getAvailableUpstreamModules"
          ...
          >
            Match federated fields with existing ones:
...
export default {
  ...
  computed: {
    servers () {
      const fields = [
        { name: 'TestField', label: 'Test Field', value: false, map: null },
        { name: 'TestField2', label: 'Test Field2', value: false, map: null },
      ]

      return [
        { federationID: '0', name: 'Bromley High School', url: 'bromley.com', email: 'bromley@mail.com', enabled: 'True', status: 'Paired', labels: ['Education', 'HR'], createdAt: new Date(), fields },
        { federationID: '1', name: 'Northwood College for Girls', url: 'northwood.com', enabled: 'True', status: 'Paired', labels: ['Education'], createdAt: new Date(), fields },
      ]
    },
    ...
    getAvailableDownstreamServers () {
      return [
        { federationID: null, name: 'None' },
        ...this.servers.filter(s => s.federationID !== this.downstream.active),
      ]
    },

    getAvailableUpstreamModules () {
      return [
        { moduleID: null, name: 'Pick a federated module' },
    ...
    servers: {
      ...
      handler (servers) {
        ...
        servers.forEach(({ federationID, fields }, index) => {
          ...
          dSetting.fields.forEach(({ name }) => {
            (downstream.fields.find(f => f.name === name)).value = true
          })
          ...
          uSetting.fields.forEach(({ name, value }) => {
            (upstream.fields.find(f => f.name === name) || {}).value = true
          })

          this.downstream[federationID] = downstream
          this.upstream[federationID] = upstream
        })
      },
    },
  },
